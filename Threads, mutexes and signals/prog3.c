...
typedef struct argsSignalHanlder
{
    pthread_t tid;
    int *pArrayCount;
    int *array;
...
int main(int argc, char**argv)
{
    ...
    if(pthread_sigmask(SIG_BLOCK, &newMask, &oldMask))
        ERR("SIG_BLOCK error");
    argsSignalHanlder_t args;
    args.array = array;
    args.pArrayCount = &arrayCount;
    args.pMask = &newMask;
    args.pmxArray = &mxArray;
    args.pmxQuitFlag = &mxQiutFlag;
    args.pQuitFlag = &quitFlag;
    if(pthread_create(&args.tid,NULL,signal_handling,&args))
        ERR("Couldn't creat signal handling thread!");
    
    while(true)
    {
        pthread_mutex_lock(&mxQiutFlag);
        if(quitFlag == true)
        {
            pthread_mutex_unlock(&mxQiutFlag);
            break;
        }else
        {
            pthread_mutex_unlock(&mxQiutFlag);
...
{
    argsSignalHanlder_t *args = voidArgs;
    int signo;
    srand(time(NULL));
    for(;;)
    {
